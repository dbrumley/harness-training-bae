...
{
	$location_readonly = 1;
	if (exec("/bin/af get gpsfix | sed -r -e 's/\n//'") != "no") {

		$latitude = exec("/bin/af get gpslat | sed -r -e 's/\n//'");
		if (strtoupper($latitude) != "unavailable") {
			$dir = ereg_replace("[^NS]+","",$latitude);
			$latitude = doubleval(ereg_replace("[NS]","",$latitude));
			$latitude = doubleval(intval($latitude/100)) + (doubleval($latitude - (intval($latitude/100)*100)) / 60);
			if (strtoupper($dir) == "S") {
					$latitude = $latitude * -1;
			}
		}

		$longitude = exec("/bin/af get gpslon | sed -r -e 's/\n//'");
		if (strtoupper($longitude) != "unavailable") {
			$dir = ereg_replace("[^EW]+","",$longitude);
			$longitude = doubleval(ereg_replace("[NS]","",$longitude));
			$longitude = doubleval(intval($longitude/100)) + (doubleval($longitude - (intval($longitude/100)*100)) / 60);
			if (strtoupper($dir) == "W") {
