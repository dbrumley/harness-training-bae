...
Fuzzing networked targets is challenging because it raises the question
how does one control the data sent over the socket?  At ForAllSecure we
solve this by emulating all network calls.  This lab also also dives
into some of the more complex aspects of harnessing.  The lab requires
the user to correctly identify the correct modules and configuration
file required by the target.
...
We have a configuration file ready `/tmp/nginx.conf`.  Copy this to
...
### Exercise: Lighttpd
There are two variants of the lighttpd tutorial: moderate and hard.
